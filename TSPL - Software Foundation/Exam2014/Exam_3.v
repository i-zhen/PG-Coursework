...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileLoop : forall st st' st'' b c,
      ...
      (WHILE b DO c END) / st' || st'' ->
      (WHILE b DO c END) / st || st''
  | E_LoopEnd : forall c1 b c2 st st',
      c1 / st || st' ->
      beval st' b = false ->
      (LOOP c1 WHILE b DO c2 END) / st || st'
  | E_LoopLoop : forall c1 st st' b c2 st'' st''',
      c1 / st || st' ->
      beval st' b = true ->
      c2 / st' || st'' ->
      (LOOP c1 WHILE b DO c2 END) / st'' || st''' ->
...
Theorem hoare_loop: forall P Q b c1 c2,
  {{P}} c1 {{Q}} ->
  {{fun st => Q st /\ bassn b st}} c2 {{P}} ->
  {{P}} LOOP c1 WHILE b DO c2 END {{fun st => Q st /\ ~(bassn b st)}}.
Proof.
  intros P Q b c1 c2 Hhoare H st st' He HP.
  remember (LOOP c1 WHILE b DO c2 END) as wcom eqn:Heqlcom.
  induction He;
