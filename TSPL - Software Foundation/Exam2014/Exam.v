...
Fixpoint subst (x:id) (s:tm) (t:tm) : tm :=
  ...
  | tsucc t =>
   ...
   | tncase t1 t2 x' t3 =>
       tncase ([x:=s] t1)
         ([x:=s] t2)
          x' (if beq_id x x' then t3 else ([x:=s] t3))
  end
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_If : forall t1 t1' t2 t3,
   ...
   | ST_Succ : forall t t',
       t ==> t' ->
      (tsucc t) ==> (tsucc t')
    | ST_Ncase : forall x t1 t1' t2 t3,
      t1 ==> t1' ->
      (tncase t1 t2 x t3) ==> (tncase t1' t2 x t3)
    | ST_NcaseZero : forall t2 t3 x,
       (tncase tzero t2 x t3) ==> t2
     | ST_NcaseSucc : forall x v1 t2 t3,
       value v1 ->
       (tncase (tsucc v1) t2 x t3) ==> [x:=v1] t3
  ...
  | T_If : forall t1 t2 t3 T Gamma,
   ...
   | T_Zero : forall Gamma,
    ...
    | T_Succ : forall t Gamma,
     ...
     | T_Ncase : forall x t1 t2 t3 T Gamma,
          Gamma |- t1 \in TNat ->
          Gamma |- t2 \in T ->
         update Gamma x TNat |- t3 \in T ->
          Gamma |- (tncase t1 t2 x t3) \in T
...
Proof with eauto.
  ...
  + (* Case "T_If" *)
    ...
    + destruct IHHt...
       right. inversion H as [t' Hstp]. exists(tsucc t')...

    + right. destruct IHHt1...
       - inversion H; subst; try solve by inversion...
       - inversion H as [t1' Hstp]. exists (tncase t1' t2 x t3)...
Qed.
