...
Inductive suffix {X : Type} : list X -> list X -> Prop :=
  | suffix_1 : forall xs,
     suffix xs xs
  | suffix_2 : forall x xs ys,
     suffix xs ys -> suffix (x::xs) (ys).
...
Proof.
  ...
  - inversion H. subst.
   ...
   + inversion H. subst.
      * exists([x0]). simpl. rewrite H4. reflexivity.
      * subst. inversion IHsuffix. 
        exists(x0::x3). rewrite H1. simpl. reflexivity.
Qed.

Inductive member {X : Type} : X -> list X -> Prop :=
...
Proof.
  intros.
  induction H.
  + simpl. apply member_here.
  + simpl. apply member_later.
     apply IHmember.
Qed.
...
Proof. 
  intros.
  inversion H.
  + generalize dependent H0. apply app_member_left.
  + generalize dependent H0. apply app_member_right.
Qed.
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_LoopLoop : forall st st' st'' b c,
        c / st || st' ->
        beval st' b = false ->
        (REPEAT c UNTIL b END) / st' || st'' ->
        (REPEAT c UNTIL b END) / st || st''
...
Tactic Notation "ceval_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "E_RepeatEnd" | Case_aux c "E_LoopLoop" ].
...
Tactic Notation "t_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "tbranch" | Case_aux c "tcase"
  | Case_aux c "tleaf" ].
...
Fixpoint subst (x:id) (s:tm) (t:tm) : tm :=
  ...
  | tcase t1 t2 xt y zt t3 =>
      tcase ([x:=s] t1) ([x:=s] t2) xt y zt
      (if eq_id_dec x xt then 
            t3 
        else if eq_id_dec x y then 
            t3 
        else if eq_id_dec x zt then 
            t3 
        else 
            ([x:=s] t3))
  | tleaf =>
...
Inductive step : tm -> tm -> Prop :=
  ...
  (* !!! *)
  | ST_BranchLeft : forall t1 t2 t3 t1',
       t1 ==> t1' ->
      (tbranch t1 t2 t3) ==> (tbranch t1' t2 t3)
  | ST_BranchMiddle : forall v1 t2 t3 t2',
       value v1 ->
       t2 ==> t2' ->
      (tbranch v1 t2 t3) ==> (tbranch v1 t2' t3)
  | ST_BranchRight : forall t3 v1 v2 t3',
       t3 ==> t3' -> value v1 -> value v2 ->
      (tbranch v1 v2 t3) ==> (tbranch v1 v2 t3')
  | ST_TCase : forall t1 t1' t2 t3 xt y zt,
       t1 ==> t1' ->
      (tcase t1 t2 xt y zt t3) ==> (tcase t1' t2 xt y zt t3)
  | ST_TCaseLeaf : forall t2 t3 xt y zt,
      (tcase tleaf t2 xt y zt t3) ==> t2
  | ST_TCaseBranch : forall v11 v12 v13 t2 t3 xt y zt,
       value v11 -> value v12 -> value v13 ->
      (tcase (tbranch v11 v12 v13) t2 xt y zt t3) ==> ([xt := v11]([y := v12]([zt:=v13] t3)))
...
Tactic Notation "step_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "ST_BranchLeft" | Case_aux c "ST_BranchMiddle"
  | Case_aux c "ST_BranchRight" | Case_aux c "ST_TCase"
...
Inductive has_type : context -> tm -> ty -> Prop :=
  ...
  (* !!! *)
  | T_Leaf : forall Gamma T,
        Gamma |- tleaf \in (TTree T)
  | T_Branch : forall Gamma T t1 t2 t3,
        Gamma |- t1 \in (TTree T) ->
        Gamma |- t2 \in T ->
        Gamma |- t3 \in (TTree T) ->
        Gamma |- (tbranch t1 t2 t3) \in (TTree T)
  | T_TCase : forall Gamma t0 t1 t2 T T' xt y zt,
        Gamma |- t0 \in (TTree T) ->
        Gamma |- t1 \in T' ->
        extend (extend (extend Gamma xt (TTree T)) y T) zt (TTree T)
               |- t2 \in T' ->
  ...
  Case "T_Branch".
    destruct IHHt1...
       SCase "t1 is a value".
          ...
          SSCase "t2 is also a value".
            destruct IHHt3...
                SSSCase "t3 is a step".
                right. inversion H1.  exists (tbranch t1 t2 x)...
          SSCase "t2 steps".
            inversion H0.
            right. exists (tbranch t1 x t3)...
        SCase "t1 steps".
        right. inversion H. exists (tbranch x t2 t3)...
   ...
   Case "T_TCase".
       ...
       SCase "t0 steps".
           right. inversion H. exists (tcase x t1 xt y zt t2)...
Qed.
