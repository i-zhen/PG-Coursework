...
Proof.
  ...
  + intros. inversion H. subst.
     - apply E_IfFalse. simpl. 
       * apply negb_false_iff. apply H5.
       * apply H6.
     - apply E_IfTrue. simpl.
       * apply negb_true_iff. apply H5.
       * apply H6.
  + intros. inversion H. subst.
...
(**  AAA: This axiom will now be introduced in Logic.v; we should
   change this discussion accordingly. 
 *)
...
(** Here we're stuck. The goal looks reasonable, but in fact it is not
    ...
    [t_update] in the last chapter, we can see that lemma [t_update_same]
...
(** What is going on here?  Recall that our states are just
    ...
    [t_update] operation, this means that two functions can be proven
    equal only if they were constructed using the _same_ [t_update]
...
Proof.
  ...
  + inversion H2. subst.
     - apply E_IfTrue.
       * rewrite <- H8. rewrite H. reflexivity.
       * apply H0. apply H9.
     - apply E_IfFalse.
       *  rewrite <- H8. rewrite H. reflexivity.
       *  apply H1. apply H9.
