...
(* The inititial number of T is meanless *)
Definition T := Id 999.
...
Proof.
  unfold swap_program.
  apply hoare_seq with (Q := fun st : state => st T <= st Y).
 + apply hoare_seq with (Q:= fun st: state => st T <= st X).
     * eapply hoare_consequence_pre. apply hoare_asgn.
        intros st H. apply H.
     * eapply hoare_consequence_pre. apply hoare_asgn.
        intros st H. apply H.
...
(**  add a note about what leb_iff_conv is (from standard lib) 
 *)
...
(* refer to Imp.v about Tactic Notation*)
...
Proof.
  ...
  + split.
     apply H with (st := st). apply H0. apply H1.
     apply bexp_eval_true in H2.
     apply H2.
  + apply IHceval2.
      reflexivity.
      apply H with (st := st).
...
(** $Date: 2016-02-01 20:04:08 +0000 (Mon, 01 Feb 2016) $ *)
