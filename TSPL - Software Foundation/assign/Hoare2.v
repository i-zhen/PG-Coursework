...
(** Recall that [n!] denotes the factorial of [n] (i.e. [n! =
  ...
  END
   ...
   NOTE : 0! = 1
   The loop invariant is (Y * X! = m!) here.
*)
...
Lemma fact_loop_invariant: forall m,
...
Proof.
  intros.
  verify.
  + rewrite <- mult_assoc. rewrite  fact_loop_invariant.
     apply H. apply H0.
  + simpl in H. 
...
(* ####################################################### *)
(**  add a section header and a better intro, now that it's
   uncommented... 
 *)

(**  This hidden part contains formal decorated programs corresponding
   to most of the informal ones in the chapter *)

(* The simple examples from the decorated programs section *)
...
(* Much simpler way to compute parity, due to Daniel Cristofani. *)

(**  This might be better phrased in terms of the boolean even test
...
(* The old way to phrase the invariant (not simple enough!) *)

Definition find_parity'_dec m : dcom :=
 ({{ fun st => st X = m}} ->>
...
Theorem find_parity'_correct : forall m,
  dec_correct (find_parity'_dec m).
Proof.
  intro m. verify;
    ...
    - (* invariant preserved *)
      ...
      apply l2; assumption.
    - (* invariant strong enough to imply conclusion (-> direction) *)
      rewrite <- minus_n_O in H2. assumption.
    - (* invariant strong enough to imply conclusion (<- direction) *)
      destruct (st X) as [| [| n]]. (* by H1 X can only be 0 or 1 *)
      + (* st X = 0 *) reflexivity.
      + (* st X = 1 *) apply l3 in H; try assumption. inversion H.
      + (* st X = 2 *) clear H0 H2. omega. (* omega confused otherwise *)
Qed.

(* Same program as find_parity, invariant written differently
   (more intuitive) *)
Definition find_parity'_dec' m : dcom :=
 ({{ fun st => st X = m}} ->>
  {{ fun st => ev (st X) <-> ev m }}
...
Proof.
  ...
  - (* invariant strong enough to imply conclusion (<- direction) *)
      ...
      + (* st X = 0 *) reflexivity.
      + (* st X = 1 *) inversion H.
      + (* st X = 2 *) clear H0 H H3. omega. (* omega confused otherwise *)
Qed.

(* The simplest invariant for parity *)
...
(* Simplest variant [square_simpler_dec] at the end *)
...
Proof.
  intro n. verify.
  - (* invariant preserved *)
    destruct (st Y) as [| y']. apply False_ind. apply H0. reflexivity.
    simpl. rewrite <- minus_n_O.
    assert (G : forall n m, n * S m = n + n * m).
      clear. intros. induction n. reflexivity. simpl. rewrite IHn. omega.
...
Definition square_dec' (n : nat) : dcom := (
  ...
  {{ fun st => st Z = st X * (st X - st Y)
               /\ st X = n /\ st Y <= st X}}
  WHILE BNot (BEq (AId Y) (ANum 0)) DO
    {{ fun st => (st Z = st X * (st X - st Y) /\ st X = n /\ st Y <= st X)
                 /\ st Y <> 0 }}
    Z ::= APlus (AId Z) (AId X)
...
Proof.
  ...
  - (* invariant preserved *) subst.
    ...
    destruct (st Y). apply False_ind. apply H0. reflexivity.
      clear. rewrite mult_succ_r. rewrite plus_comm. apply le_plus_l.
...
Proof.
  intro m. verify.
  rewrite mult_plus_distr_r. simpl. rewrite <- plus_n_O. reflexivity.
Qed.
...
Proof.
  ...
  - (* 2 *)
    ...
    rewrite <- pow2_plus_1. replace (n + 1 + 1) with (n + 2) by omega.
    reflexivity.
  - (* 3 *)
...
(** $Date: 2016-01-25 19:32:29 +0000 (Mon, 25 Jan 2016) $ *)
