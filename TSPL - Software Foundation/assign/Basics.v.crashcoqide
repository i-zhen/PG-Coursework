...
Proof.
  ...
  (* rewtire -> f true = true twice*)
  - rewrite -> x, x.
    reflexivity.
 (* rewtire -> f false = false twice*)
  - rewrite -> x, x. 
    reflexivity.
Qed.
...
Proof.
  intros f x b. destruct b.
  (* rewtire -> f true = false and then f false = true *)
  - rewrite -> x, x.
    reflexivity.
 (* rewtire -> f false = true and then f true = false*)
  - rewrite -> x, x. 
    reflexivity.
Qed.
(** [] *)
...
Example test_bin_incr2 : bin_to_nat(incr(B (M (B (M O))))) = 7.
Proof. simpl. reflexivity. Qed.

Example test_bin_incr3 : S (bin_to_nat(B (M (B (M O))))) = 7.
Proof. simpl. reflexivity. Qed.

Example test_bin_incr4 : S (bin_to_nat(M (B O)))= 2.
Proof. simpl. reflexivity. Qed.

Example test_bin_incr5 :  bin_to_nat(incr(B (B (B (B (B (B (M O))))))))= 65.
Proof. simpl. reflexivity. Qed.
...
(** $Date: 2016-01-07 13:44:20 +0000 (Thu, 07 Jan 2016) $ *)
