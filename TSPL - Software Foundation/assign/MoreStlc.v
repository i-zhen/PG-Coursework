...
Fixpoint subst (x:id) (s:tm) (t:tm) : tm :=
  ...
  (* FILL IN HERE*)
  (* !!! *)
  | tlet y t1 t2 => tlet y (subst x s t1) (if beq_id x y then t2 else (subst x s t2))
  | tinl T t1 =>
      tinl T (subst x s t1)
  | tinr T t1 =>
      tinr T (subst x s t1)
...
Inductive step : tm -> tm -> Prop :=
  ...
  (* fix *)
   | ST_Fix1 : forall t1 t1',
        t1 ==> t1' ->
        (tfix t1) ==> (tfix t1')
   | ST_FixAbs : forall x T1 t2,
        (tfix (tabs x T1 t2)) ==> (subst x (tfix (tabs x T1 t2)) t2)
   (* !!! *)
   (* empty *)
   | ST_Empty : forall t t' ,
        t ==> t' ->
...
(* FILL IN HERE *)
  ...
  - (* fix *)
    ...
    + reflexivity.
    + inversion H; subst; try solve by inversion.
       exists (subst x (tfix (tabs x T11 t12)) t12)...
    + destruct H as [t1' H]. exists (tfix t1')...
  - (* empty *)
    (* !!! *)
    right. destruct IHHt.
    + reflexivity.
    + inversion H; subst; try solve by inversion.
    + destruct H as [t1' H]. exists (tempty t1')...
(* FILL IN HERE *)
Qed.
...
Inductive appears_free_in : id -> tm -> Prop :=
  ...
  (* let *)
  (* !!! *)
  | afi_let1 : forall x id t1 t2,
     appears_free_in x t1 ->
     appears_free_in x (tlet id t1 t2)
  | afi_let2 : forall x id t1 t2,
     id <> x ->
     appears_free_in x t2 ->
     appears_free_in x (tlet id t1 t2)
(* FILL IN HERE *)
  ...
  (* empty *)
   | afi_empty : forall x t,
     appears_free_in x t ->
     appears_free_in x (tempty t)
(* FILL IN HERE *)
  .

Hint Constructors appears_free_in.

(* The following two theorems are copied from SfLib.v.
    I have to do this or I cannot use them directly. 
   I think this is a bug of Coq 8.4
 *)
...
Proof with eauto.
  ...
  - (* let *)
    ...
    apply IHhas_type2...
    intros x0 Hafi. unfold update, t_update.
...
(* FILL IN HERE *)
  ...
  - (* left *)
    ...
    unfold update, t_update in Hctx. rewrite false_beq_id in Hctx...
  - (* right *)
    ...
    (* !!! *)
      ...
      eapply T_Let...
       + eapply context_invariance... intros.
          unfold update, t_update. subst.
          destruct (beq_idP i x0)...
