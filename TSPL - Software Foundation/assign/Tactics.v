(** * Tactics: Additional Basic Coq Tactics *)
...
Proof.
  ...
  (* At this point, we could finish with 
     ...
     done several times before. But we can achieve the
...
(** Coq provides a tactic called [inversion] that allows us to
    exploit these principles in proofs. To understand how to use it,
    we can work through a few examples. Let's show that the [S]
...
(** It is not clear how we can use the tactics we've seen to
    ...
    replacing variables in the goal as it goes. In this example, this
    amounts to adding a new hypothesis [H1 : n = m] and replacing [n]
    by [m]. *)

  inversion H. reflexivity.  Qed.

(** Note that injectivity works for arbitrary constructors, not
    just [S]. Here's a more interesting example, which shows how
...
(** However, the second one doesn't look so simple: we must show
    that [S n' = 0], which is clearly false! The solution lies in the
    hypothesis. After simplifying the goal, we see that [beq_nat 0 (S
    n') = true] becomes [false = true]: *)

  - (* n = S n' *)
    simpl.

(** When we use [inversion] on this hypothesis, Coq notices that
    this subgoal is impossible, and therefore removes it from further
    consideration. *)
...
(** This is an instance of a more general logical principle
    ...
    contradiction entails anything, even false propositions. For
    instance: *)
...
(** If you find the principle of explosion confusing, remember
    ...
    situation described by the premises can never arise, hence we
    don't have to prove anything. We'll explore the principle of
    explosion of more detail in the next chapter. *)
...
(**  Here, [unfold] appears for the first time...  We should
   explain it either right here (suboptimal) or earlier in this
   chapter (better, but we need to cons up a good small example). 
 *)
...
Proof.
  ...
  - (*forall m : nat, beq_nat (S n) m = beq_nat m (S n)*)
    ...
    + (*beq_nat (S n) (S m) = beq_nat (S m) (S n)
        IHn : forall m : nat, beq_nat n m = beq_nat m n*)
        apply IHn.
Qed.
...
Proof.
  ...
  rewrite H0 in H.
  rewrite H.
  assert (H1 : forall x, beq_nat x x = true).
     { induction x as [| x']. 
         + reflexivity.
         + simpl. apply IHx'.
      }
...
Fixpoint forallb {X} (f : X -> bool) ( l : list X) :=
  match l with
    | [ ] => true
    | h :: t => match f h with
                   | true => forallb f t
...
Example forall_ex1 : forallb oddb [1;3;5;7;9] = true.
Proof.
  simpl.
  reflexivity.
Qed.

Example forall_ex2 : forallb negb [false;false] = true.
Proof.
  simpl.
  reflexivity.
Qed.

Example forall_ex3 : forallb evenb [0;2;4;5] = false.
Proof.
  simpl.
  reflexivity.
Qed.

Example forall_ex4 : forallb (beq_nat 5) [ ] = true.
Proof.
  simpl.
  reflexivity.
Qed.

Fixpoint existsb {X} (f : X -> bool) ( l : list X) :=
  match l with
    | [ ] => false
    | h :: t => match f h with
                   | true => true
...
Example ex_ex1:  existsb (beq_nat 5) [0;2;3;6] = false.
Proof. simpl. reflexivity. Qed.

Example ex_ex2:  existsb (andb true) [true;true;false] = true.
Proof. reflexivity. Qed.

Example ex_ex3:   existsb oddb [1;0;0;0;0;3] = true.
Proof. reflexivity. Qed.

Example ex_ex4:    existsb evenb [] = false.
Proof. simpl. reflexivity. Qed.
...
Proof.
  intros.
  (*We will get stuck if we unfold existsb
    Both of existsb and forall are recursive, thus it is better to unfold existsb'.*)
  unfold existsb'.
  induction l.
  - (*existsb f [] = negb (forallb (fun x : X => negb (f x)) [])*)
    reflexivity.
  - (*existsb f (x :: l) = negb (forallb (fun x0 : X => negb (f x0)) (x :: l))*)
    simpl.
    (*f x can be either true or false, destruct it*)
    destruct (f x).
    + (*f x : true*)
      simpl. reflexivity.
    + (*f x : false*)
...
(** $Date: 2016-01-04 21:20:03 +0000 (Mon, 04 Jan 2016) $ *)