...
(** There is some annoying repetition in this proof.  Each use of
    ...
    relevant (the one which matches the current case in the induction
    ...
    There is a tactic called [solve by inversion] defined in a small
    accompanying file [SfLib.v] that can be of use in such cases. It
    will solve the goal if it can be solved by inverting some
    hypothesis; otherwise, it fails.  (There are variants [solve by
...
Proof.
  ...
  + apply multi_trans with (y := P (C n1) (C n2)).
     * apply multi_trans with (y := P (C n1) t2).
        - generalize dependent IHeval1.
          apply multistep_congr_1.
        - generalize dependent IHeval2.
           apply multistep_congr_2.
          constructor.
      * apply multi_step with (y :=  (C (n1 + n2))).
        - apply ST_PlusConstConst.
        - apply multi_refl.
Qed.
...
Proof.
  ...
  + reflexivity.
  + induction H.
     - rewrite <- H1.  apply E_Const.
     - apply step__eval with (t' := y). 
        * apply H.
        * apply IHmulti. assumption. assumption.
Qed.
...
(** $Date: 2016-02-04 21:50:25 +0000 (Thu, 04 Feb 2016) $ *)
