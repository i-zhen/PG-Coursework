...
Proof.
  intros m n o H1 H2.
  induction H2 as [ | o' IHo'].
  (*m <= n by the definition le_n*)
  - apply H1.
  (*m <= S o' by the definition le_S*)
  - apply le_S. apply IHIHo'.
Qed. 
...
Proof.
  intros n m H.
  induction H as [| m' IHm'].
  (* S n <= S n *)
  - apply le_n.
  (* S n <= S (S m') *)
  - apply le_S. apply IHIHm'.
Qed.
...
Proof.
  ...
  - (*a <= a + 0*) 
    ...
    { induction a as [|a' IHa']. 
      + (*0 + 0 = 0*) reflexivity. 
      + (*S a' + 0 = S a'*) simpl. rewrite -> IHa'. reflexivity.
    }
...
Lemma S_nm_le_Sn : forall n m x:nat,
  S (n + m) <= x -> S m <= x.
Proof. 
  intros.
  induction H.
  - destruct m.
    ...
    + apply n_le_m__Sn_le_Sm.
       induction n.
       * rewrite plus_O_n. apply le_n.
       * rewrite plus_Sn_m. apply le_S. apply IHn.  
...
Proof.
  ...
  - induction n2.
    ...
    + apply le_S. rewrite plus_comm, plus_Sn_m.
       rewrite plus_comm. apply IHn2.
  - apply le_S. apply IHle.
  - generalize dependent H. apply S_nm_le_Sn.
Qed.
...
(** **** Exercise: 4 stars, optional (star_app)  *)
...
Inductive pal {X:Type} : list X -> Prop := 
    | emptyl : forall l, length l = 0 -> pal l
    | otherl   : forall l, (lt 0 (length l)) /\ (l = rev l) -> pal l.
*)

Inductive pal {X:Type} : list X -> Prop :=
    | emptyl : pal []
    | onel     : forall x, pal [x]
    | twol     : forall x l, pal l -> pal (x :: l ++ [x]).

Lemma one_le_S : forall n, 1 <= S n.
Proof.
  intros.
  induction n.
  - apply le_n.
...
Proof.
  ...
  induction l.
  - simpl. apply emptyl. 
  - simpl. rewrite  app_assoc. apply twol.
    apply IHl.
Qed.
(*
Lemma len_zero : forall (X : Type) (l : list X) , length l  = 0 -> l = rev l.
Proof.
  intros.
  induction l.
  - reflexivity.
...
Lemma len_zero' : forall (X : Type) (l : list X) , length l  = 0 -> l = [].
Proof.
  intros.
  induction l.
  - reflexivity.
...
Lemma list_plus_empty : forall (X:Type)(a: list X),
  a ++ [] = a.
Proof.
  intros.
  induction a as [| n l IHl'].
  - reflexivity.
  - simpl. rewrite -> IHl'. reflexivity.
Qed.

Theorem rev_demorgan : forall (X:Type)(a b : list X),
  rev (a ++ b) = (rev b) ++ (rev a).
Proof.
  intros.
  induction a as [| n l' IHl'].
 - (*rev ([] ++ b) = rev b ++ rev [] *)
  simpl. rewrite -> list_plus_empty. reflexivity.
 - (* rev ((n :: l') ++ b) = rev b ++ rev (n :: l') *)
...
- (*rev (rev []) = [] *)
  simpl. reflexivity.
- (* (n :: l') ++ [] = n :: l' *)
  simpl. rewrite ->  rev_demorgan, IHl'. reflexivity.
Qed.

Lemma len_one : forall (X : Type) (l : list X) , length l  = 1 -> l = rev l.
Proof.
  intros.
  induction l.
  - reflexivity.
  - inversion H.
    apply len_zero in H1.
    rewrite H1. simpl. rewrite rev_involutive.
    rewrite H1. rewrite rev_involutive.
    inversion H.
    apply  len_zero'  in H2. rewrite H2.
    reflexivity.
Qed.
*)
...
(** First prove an easy useful lemma (we already proved it for lists
