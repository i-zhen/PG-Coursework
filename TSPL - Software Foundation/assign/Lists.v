...
Lemma list_plus_empty : forall a : natlist,
  a ++ [] = a.
Proof.
  intros a.
...
Theorem rev_demorgan : forall a b : natlist,
  rev (a ++ b) = (rev b) ++ (rev a).
Proof.
  intros a b.
  induction a as [| n l' IHl'].
...
Proof.
 ...
 - (* nonzeros ([] ++ l2) = nonzeros [] ++ nonzeros l2 *)
  simpl. reflexivity.
 - (* nonzeros ((n :: l') ++ l2) = nonzeros (n :: l') ++ nonzeros l2 *)
  destruct n. 
    (* nonzeros ((0 :: l') ++ l2) = nonzeros (0 :: l') ++ nonzeros l2 *)
    + simpl. rewrite -> IHl'. reflexivity.
    (* nonzeros ((S n :: l') ++ l2) = nonzeros (S n :: l') ++ nonzeros l2 *)
    + simpl. rewrite -> IHl'. reflexivity.
Qed.
(** [] *)
...
Lemma beq_nat_true : forall n : nat,
  beq_nat n n = true.
Proof.
  intros.
...
Proof. 
  intros l. induction l as [| n l' IHl']. 
  - reflexivity.
  - simpl. rewrite -> IHl'. rewrite -> beq_nat_true.
    reflexivity.
Qed.
(** [] *)
...
Proof.
  intros.
  induction l as [| n l' IHl'].
  - (*  hd default [] = option_elim default (hd_error []) *)
    simpl. reflexivity.
  - (* hd default (n :: l') = option_elim default (hd_error (n :: l')) *)
    simpl. reflexivity.
Qed.
...
(** $Date: 2016-01-05 13:55:01 +0000 (Tue, 05 Jan 2016) $ *)
