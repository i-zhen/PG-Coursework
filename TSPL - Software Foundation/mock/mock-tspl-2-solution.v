...
| break_eq : forall x xs,
    break (x :: xs) x xs
| break_cons : forall x xs y ys,
    break xs y ys ->
    break (x :: xs) y (x :: ys).

(* ii *)

Lemma break_a : break [1;2;3] 1 [2;3].
Proof. apply break_eq. Qed.

Lemma break_b : break [1;2;3] 2 [1;3].
Proof. apply break_cons. apply break_eq. Qed.

Lemma break_c : break [1;2;3] 3 [1;2].
Proof. apply break_cons. apply break_cons. apply break_eq. Qed.
...
Proof. 
  ...
  - inversion H0.
    ...
    + right. assumption.
  - inversion H0.
    + right. rewrite -> H1. apply in_eq.
    + apply IHbreak in H1. inversion H1.
      * left. assumption.
...
Lemma hoare_for : forall P Q X a1 a2 c,
  {{P}} LOOP X TO a2 DO c END {{Q}} ->
  {{P[X |-> a1]}} FOR X == a1 TO a2 DO c END {{Q}}.
Proof.
  intros P Q X a1 a2 c Hhoare st st' He HP.
  inversion He.  subst. apply (Hhoare (t_update st X (aeval st a1)) st').
  - assumption.
  - unfold assn_sub in HP. assumption.
Qed.
...
(*
            {{P /\ X <= a2}} c {{P [X |-> X + 1]}}
     -------------------------------------------- hoare_loop
     {{P}} LOOP X TO a2 DO c END {{P /\ X > a2 }}
*)

Lemma hoare_loop : forall P X a2 c,
  {{fun st => P st /\ aeval st (AId X) <= aeval st a2}} c {{P[X |-> (APlus (AId X) (ANum 1))]}} ->
  {{P}} LOOP X TO a2 DO c END {{fun st => P st /\ aeval st (AId X) > aeval st a2}}.
Proof.
  intros P X a2 c Hhoare st st' He HP.
  remember (LOOP X TO a2 DO c END) as wcom eqn:Heqwcom.
  induction He;
    try (inversion Heqwcom); subst; clear Heqwcom.
...
Proof with eauto.
  ...
  + (* Case "T_Snoc" *)
      destruct IHHt1...
      - (* t1 is a value *) destruct IHHt2. reflexivity.
         * (* t2 is a value *) left...
         * (* t2 steps *) right. inversion H0 as [t2' Hstep2]. exists (tsnoc t1 t2')...
